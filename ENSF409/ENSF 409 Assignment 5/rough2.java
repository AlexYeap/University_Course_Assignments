...
public class VisitorParking {
    ...
    public VisitorParking(String license){
        ...
        LocalDate currentDate = LocalDate.now();
        if(plates.get(license) != ""){
            if(!licensePlate.contains(license)){
                if(duplicatePlates2.get(license).get(0) != null){
                    this.licensePlate.add(license);
                    numberOfRes++;
                    plates.put(license, "");
                    duplicatePlates3.computeIfAbsent(license, k -> new ArrayList<>()).add(currentDate.toString());
                }
                else{
                this.licensePlate.add(license);
                numberOfRes++;
                plates.put(license, "");
                duplicatePlates2.computeIfAbsent(license, k -> new ArrayList<>()).add(currentDate.toString());
                }
