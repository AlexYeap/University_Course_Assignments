...
{
  while(rhs.cursor_ok()){
    os << rhs.cursor_key() << " " << rhs.cursor_datum().c_str() << "\n";
    rhs.step_fwd();
  }
  rhs.go_to_first();
  return os;
...
char& DictionaryList::operator [](int index)const
{

  assert(index >= 0 && index < cursor_datum().length());

  return this->cursor_datum()[index];
}
...
{
  this->headM = 0;
  Node *next = source.headM;
  while (next != 0){
    this->insert(next->keyM, next->datumM);
    next = next->nextM;
  }
